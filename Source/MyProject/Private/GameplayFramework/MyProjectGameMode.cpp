...
#include "GameplayFramework/MyProjectGameMode.h"
#include "GameplayFramework/MyProjectCharacter.h"
...
void AMyProjectGameMode::PostLogin(APlayerController* NewPlayer)
{
	...
	if (auto MyController = Cast<AMyPlayerController>(NewPlayer))
	{
		PlayerControllers.AddUnique(MyController);
	}
}
void AMyProjectGameMode::Logout(AController* Exiting)
{
	Super::Logout(Exiting);
	if (auto MyController = Cast<AMyPlayerController>(Exiting))
	{
		PlayerControllers.Remove(MyController);
	}
}